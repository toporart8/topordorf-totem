 ...
 */
function getSlavicHall(day, month) {
    // Вспомогательная функция для перевода даты в "день года" (условно), 
    // но проще сравнивать границы месяцев.
    ...
    for (const hall of SLAVIC_HALLS) {
        ...
        // Случай 1: Период внутри одного месяца (напр. 3.11 - 24.11)
        if (start.m === end.m) {
            if (month === start.m && day >= start.d && day < end.d) {
                isMatch = true;
            }
        }
        // Случай 2: Период внутри года, разные месяцы (напр. 27.08 - 20.09)
        ...
        // Случай 3: Переход через год (напр. 16.12 - 07.01)
        else {
            if ((month === start.m && day >= start.d) ||
                (month === end.m && day < end.d) ||
                (month > start.m) ||
 ...
 * 
 * ВАЖНО: Базовый год смещен на 1939, чтобы соответствовать системе, 
 * где 1980 год считается годом Сороки (Index 9).
 * (1980 - 1939) = 41. 41 % 32 = 9 (Сорока).
 * До 21 марта 1980 -> это 1979 год по зороастрийскому календарю.
 * (1979 - 1939) = 40. 40 % 32 = 8 (Черепаха).
 * 
 * @param {number} year 
 * @param {number} month 
 * @param {number} day 
 * @returns {object} { totem, symbol }
 */
function getZoroastrianTotem(year, month, day) {
...
function getZodiac(day, month) {
    ...
    // Месяц 1..12. В массиве индексы 0..12.
    // Если день > end, то следующий знак.
    // Январь (1) -> смотрим индекс 0 (Козерог до 19). Если больше -> индекс 1.
    // Декабрь (12) -> смотрим индекс 11 (Стрелец до 21). Если больше -> индекс 12 (Козерог).
...
// ==========================================
// 4. ТЕСТИРОВАНИЕ (TEST CASES)
// ==========================================

function runTests() {
    console.log("=== ЗАПУСК ТЕСТОВ (LOGIC.JS) ===");

    const testDates = [
        { d: 15, m: 2, y: 1980, desc: "15 Февраля 1980 (До 21 марта -> должен быть 1979 -> Черепаха)" },
        { d: 21, m: 3, y: 1985, desc: "21 Марта (Граница зороастрийского года)" },
        { d: 10, m: 10, y: 1990, desc: "Середина осени (Проверка Чертога)" }
    ];

    testDates.forEach(date => {
        console.log(`\n--- Дата: ${date.d}.${date.m}.${date.y} (${date.desc}) ---`);

        const slavic = getSlavicHall(date.d, date.m);
        console.log(`[Славянский]: ${slavic.hall} (${slavic.god})`);

        const zoro = getZoroastrianTotem(date.y, date.m, date.d);
        console.log(`[Зороастрийский]: ${zoro.totem} (Символ: ${zoro.symbol})`);

        const zodiac = getZodiac(date.d, date.m);
        console.log(`[Зодиак]: ${zodiac}`);
    });
    console.log("\n=== ТЕСТЫ ЗАВЕРШЕНЫ ===");
}

// Запускаем тесты при прямом вызове
runTests();
